"""The vetted query functions — the only path by which data is read.

- Four questions answered by walking the ontology: what's due, what exams are
  coming, which requirements are short, and which planned courses are blocked
  by unfinished prerequisites.
- Each takes a Session and returns JSON-serializable dicts, so the dashboard,
  the HTTP layer, and the natural-language layer all share one implementation.
"""
